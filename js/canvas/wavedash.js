...
function draw() {
  ...
  if (!isDashing && !isWallJumping) {
    ...
    textSize(20);
    fill(0);
    text(`Anzahl Wavedash: ${count}`, 10, 30);

    if(count == 0) {
        image(imgs[0], 100, 100, 75, 75);
        text(`Der Wavedash ist hart, sehr schwer.. \nSpring und dashe diagonal auf den Boden, \nund wenn du landest spring direkt!`, 170, 85);
    } else if(count == 1) {
        image(imgs[1], 100, 100, 75, 75);
        text(`WOW! Wie schnell du warst!\nDu hast viel Momentum bekommen! \nSchaffst du ein weiteres mal?`, 170, 85);
    } else if(count >= 2) {
        image(imgs[2], 100, 100, 75, 75);
        text(`Du hast es nochmals geschafft! \nUnfassbar starke Leistung! 🎉🎉 \nEs ist Zeit in Celeste zu steppen..`, 170, 85);
    }
